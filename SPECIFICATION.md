...
- `due_reminders (card_id FK→cards, user_id FK→users, reminded_at DATETIME, PRIMARY KEY (card_id, user_id))` — the **claim table** that makes "once per card per recipient" true and crash-safe. One row = "this user was already reminded about this card's due date". The `PRIMARY KEY (card_id, user_id)` is the single source of dedupe truth — a second scan for the same (card, user) is an `INSERT IGNORE` no-op and never re-fires. The card→lane→board chain is already cascade-deleted, so the `card_id` FK cleans up on card delete. Migration `bin/add-due-reminders.php` (idempotent — check the table, not a column).
...
3. **Claim-then-notify (crash-safe).** For each in-window pair, `INSERT IGNORE INTO due_reminders (card_id, user_id) VALUES (?, ?)`. `affectedRows() > 0` means **this** scan won the claim for that (card, user) — fire the notification; `affectedRows() === 0` means an earlier scan (or a concurrent timer tick) already claimed it — skip. Firing is **after** the claim, so a crash between claim and notify leaves the row claimed (no re-fire) — the trade-off is "at most one reminder ever", never "double reminders". This is the right bias: re-reminding is noise, a missed reminder is a tolerable one.
...
`profile.section_due_reminders` · `profile.due_remind_hours` (the input label) · `profile.due_remind_hours_help` (the helper line) · `notification.due_reminder` (**"Card ''{0}'' is due on {1}'' — set your reminder hours on your profile page"** — `{0}`=card title ≤100 chars, `{1}`=human due date) · `flash.due_reminders_saved`. The in-app and email messages reuse `notification.due_reminder` (one string, two surfaces, NOTIF-06 contract).

**Tests (fixture-based, self-cleaning, never user id 1):**
- `tests/e2e-due-reminders.php` — service contract against a **fake Mailer** (records `send()` args; zero sockets) + the live DB: (1) **window in / out** — a card due in `N` hours with an assignee opted at exactly that N fires on scan; an assignee opted at `N+1` (window not yet reached) does **not**; (2) **past-due exclusion** — a card whose due date is yesterday is **not** reminded (second bound); (3) **archived exclusion** — an archived in-window card is skipped; (4) **completed-lane exclusion** — an in-window card in a "Done" lane and one in a "Won't fix" lane are both skipped (`isCompleteLane` parity); (5) **no-assignees skip** — a due card with zero assignees fires zero rows; (6) **dedupe (crash-safety)** — a second `scanDueReminders()` after the first fires **zero** additional rows (the `due_reminders` claim is the guard; assert `due_reminders` has exactly one row per in-window assignee and the `notifications` count does not grow on re-runs); (7) **opt-in gate** — a recipient with `email_notifications=0` gets the bell row but **no** `Mailer::send()`; one with `=1` gets exactly one `send()` with the correct `to`/`due_reminder` subject + deep link; (8) **SMTP non-fatal** — the fake Mailer that throws → the in-app `type=due` row is **still** created for every in-window assignee, the exception does **not** escape `scanDueReminders()`, and the scan continues (no partial fire); (9) **per-user independence** — two assignees at different N (1 h and 24 h) on one card: at the 24 h mark **both** are inside their window and both claimed; the test asserts each got exactly one; (10) **migration idempotency** — re-running the three `bin/add-*.php` scripts is a no-op. The fake Mailer + fixture-card/assignee setup reuses the `e2e-notification-email.php` harness shape (self-cleaning fixture user, org 1, dedicated fixture board/lane/card).
- `tests/http-due-reminders.sh` — live Apache: unauth `PUT /v1/me` with `due_remind_hours` → 403 (CSRF gate before auth, same as the theme/email/language suites); authed `PUT /v1/me {due_remind_hours:24}` → 200 row stored `24`; `{due_remind_hours:null}` → stored NULL (off); `{due_remind_hours:721}` / `{0}` / `{-1}` / `{1.5}` / `{"720"}` → **400** `due_remind_hours must be an integer between 1 and 720, or null to disable` (state unchanged each); profile page render contract (the Due-date reminders card present, input `id`, i18n keys in `data-lang`, CSS classes in app.css); **privacy-class** — a non-admin same-org viewer `GET /v1/users/4` (mya) sees `due_remind_hours:null` (nulled in the same `UserController::show()` branch as email), an admin sees the real value; and a **scan-trigger E2E**: seed a fixture card due in 2 h, opt a fixture assignee at 2 h, run `php bin/due-reminder-scan.php`, then `GET /v1/notifications` for that user shows the `type=due` row with the correct `reference_id`/`board_id` (self-cleaning).
- **Regression:** `e2e-notification-email.php` + `http-notification-email.sh` (NOTIF-06 path byte-identical — the `emitEmails`/`Mailer`/`app.url` wiring `scanDueReminders` reuses), `e2e-user-profile.php` 38/0 + `http-user-profile.sh` 38/0 (the shared `updateUser`/`PUT /v1/me` path — `theme_preference`/`email_notifications`/`language`/`phone`/`location`/`bio` byte-identical), `e2e-priority.php` + `e2e-digest.php` (the `isCompleteLane` matcher is now shared, not duplicated — re-run to confirm the done/won't-fix exclusion is identical), `http-e2e.php` 32/0, `e2e-activity.php` 34/0, `http-card-merge.sh` 12/0. `php -l` on every touched PHP file; `node --check www/js/profile.js` + `www/js/notifications.js`; `json.loads` both lang files; `bash -n` the install script + `bin/due-reminder-scan.php`; the **key-parity check** (a one-liner in the e2e suite) that `set(sv.keys) == set(en.keys)` still holds.
