...
| CARD-14 | **Single card view:** there is exactly ONE way to view and edit a card — a **feature-complete modal on the board page**. The modal carries everything the removed standalone card page had: title, due date, Markdown description (edit + preview), assignees, **checklists** (list, progress, item add/check/delete, checklist rename/add/delete), **attachments** (list, download, upload with progress, delete), **comments** (list, add, **edit and delete own comments**), and the card actions (archive/restore, **merge into…**, delete). The modal must be **vertically scrollable** (independent body scroll; header and footer stay fixed) so cards with long descriptions or many comments remain fully usable. There is no separate card detail page. **Description preview (v1.12):** the Description field offers a **Preview / Edit toggle** — Edit shows the raw Markdown textarea, Preview shows the **rendered** Markdown. The rendered HTML is produced **server-side** by a shared Markdown endpoint (`POST /v1/markdown/render`, `{markdown}` → `{html}`) using the same Parsedown safe-mode pipeline the card/comment APIs use, so **the client never parses Markdown** (the SEC-04 XSS trust chain is preserved for the preview too). The preview renders the **current textarea value at activation** (the Edit textarea is hidden while Preview is active, so there is no live-refresh — re-activate Preview to re-render), and is **read-only** — it can never be saved back; the saved value is always the raw Markdown from the Edit pane. Viewers see the Edit pane only (no toggle — their description is frozen) | Must-have |
