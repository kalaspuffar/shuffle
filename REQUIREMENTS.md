...
| PRIO-12 | **Priority digest**: the priority list page offers a **copy-digest** action that produces a short, chat-pasteable Markdown summary: the user's **top N prioritized cards** (N user-selectable, default **5**), each rendered as `emoji title — *board*` (no dead-link tail in the chat body), plus a **"Done yesterday"** section listing cards that **moved to a Done lane during the previous calendar day** (server local time), rendered as `✅ title — *board* — actor`. Cards on boards the user cannot access never appear, in either section (BOARD-04b). **Empty sections are omitted** — a "Done yesterday: (none)" heading is noise in a chat (Daniel, 2026-08-30) | Must-have |
...
| PRIO-14 | **API**: `GET /v1/priority/digest?n=&format=json\|markdown` returns the acting user's digest. `format=json` (default) returns `{n, top:[…], done_yesterday:[…]}` (each top item carries `card_html` for programmatic consumers); `format=markdown` returns the digest as a `text/markdown` body ready to paste — top items are `emoji title — *board*` (no deep-link tail in the chat body), Done-yesterday items are `✅ title — *board* — actor`, and **sections with zero items are omitted**. `n` clamps to 1–50 (default 5). Both sections empty → empty body (still HTTP 200). Board access is enforced per item (inaccessible card = omitted, never revealed). The "Done yesterday" window is `yesterday 00:00:00` to `yesterday 23:59:59` in the server's local timezone | Must-have |
